...
class game_operatoren:
	...
	def winner(self):
		...
		if 1:	###########plot
			plot=self.grid.helper_efficiency_list
			plt.hist(plot,30)
			plt.title(str(np.average(plot)))
			plt.show()
...
class game_grid: 
	...
	def extended_help(self):
		...
		pack_size=8 #größe der päckchen
		help_pack_list=[]
		...
		self.helper_efficiency_list.append(N/N2)
